...
module.exports = function (app) {
    ...
    app.get("/api/workouts", function (request, response) {
        ...
        db.WorkoutPlan.find({})
            // .populate("exercises") //why does this block the db? (records which have data don't exist here)
    ...
    //     console.log("Post Request for creating new workoutPlan: \n", request.body);
    //     db.Exercises.create(request.body)
    //         .then(function ({ _id }) {
    //             console.log("This is the id? \n" ,{_id});
    //             return db.WorkoutPlan.findOneAndUpdate({},
    //                 { $push: { WorkoutPlan: _id } }, { new: true })
    //         })
    //         .catch(error => {
    //             response.json(error);
    //         });
